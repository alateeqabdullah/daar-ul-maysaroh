...
export default async function TeacherGradesPage() {
  ...
  const avg = total > 0 ? Math.round(grades.reduce((a, b) => a + b.percentage, 0) / total) : 0;
  const topStudent = grades.reduce((prev, current) => (prev.percentage > current.percentage) ? prev : current, grades[0] || null);

  const stats: GradeStats = {
    totalGrades: total,
    averageScore: avg,
    topStudentName: topStudent?.studentName || "N/A",
    failingCount: grades.filter(g => g.grade === 'F' || g.percentage < 50).length
  };
