...
export default async function QuranPage() {
  ...
  const students: QuranStudent[] = studentsRaw.map(s => {
    ...
    if (lastLogDate) {
      const diffDays = Math.floor((new Date().getTime() - lastLogDate.getTime()) / (1000 * 3600 * 24));
      lastRecited = diffDays === 0 ? "Today" : diffDays === 1 ? "Yesterday" : `${diffDays} days ago`;
    }

    // Status logic
    let status: QuranStudent["status"] = "Active";
    if (!lastLogDate) status = "Inactive";
    else if ((new Date().getTime() - lastLogDate.getTime()) / (1000 * 3600 * 24) > 7) status = "At Risk";
