...
const navItems = [
  {
    ...
    roles: ["SUPER_ADMIN", "ADMIN", "CONTENT_MANAGER", "SUPPORT"],
  },
  {
    name: "Users",
...
// Group navigation items with role-based filtering
const getNavGroups = (role: string) => {
  ...
  for (const [groupName, items] of Object.entries(groups)) {
    const accessibleItems = items.filter((itemName) => {
      const navItem = navItems.find((ni) => ni.name === itemName);
      return navItem && navItem.roles.includes(role);
    });
    if (accessibleItems.length > 0) {
      filteredGroups[groupName] = accessibleItems;
    }
  }
...
export function AdminLayoutClient({
  ...
  const NAV_GROUPS = getNavGroups(session.user?.role || "SUPPORT");
  ...
  // Loading state while session is being processed
  if (!session) {
    return (
              ...
              <>
                <div className="flex items-center gap-3 px-3 py-2">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[10px] text-purple-600 font-medium">
                      {formatRole(session.user?.role || "ADMIN")}
                    </p>
                  </div>
                </div>
                ...
                <Tooltip delayDuration={0}>
                  <TooltipTrigger asChild>
                    <Button
                      type="submit"
                      variant="ghost"
                      className="flex items-center justify-center p-3 rounded-xl text-red-500 hover:bg-red-50 transition-colors w-full"
                      aria-label="Logout"
                    >
                      <LogOut className="w-5 h-5" />
                    </Button>
