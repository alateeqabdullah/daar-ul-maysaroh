...
// Sub-components (We assume these exist or create them next)
import { ApprovalsTable } from "@/components/dashboard/ApprovalsTable";
import { ActivityFeed } from "./admin/dashboard/_components/ActivityFeed";
import { DashboardSkeleton } from "./admin/dashboard/_components/DashboardSkeleton";
import { MainChartsSection } from "./admin/dashboard/_components/MainChartsSection";
import { StatCardsGrid } from "./admin/dashboard/_components/StatCardsGrid";
...
export default function AdminDashboardPage() {
  const { data, error, isLoading, mutate, isValidating } = useSWR<DashboardApiResponse>(
    "/api/admin/dashboard",
    fetcher,
    { 
  ...
  const handleUserAction = async (userId: string, action: "APPROVE" | "REJECT") => {
    ...
    mutate((current) => {
      ...
      return {
        ...current,
        pendingUsers: current.pendingUsers.filter(u => u.id !== userId),
        stats: { ...current.stats, pendingUsers: current.stats.pendingUsers - 1 }
      };
    }, false);
    ...
    try {
      ...
      mutate(previousData); // Rollback on error
    }
  };

  if (isLoading) return <DashboardSkeleton />;
  if (error) return <div className="p-10 text-center">Error loading dashboard.</div>;
  if (!data) return null;
    ...
    <div className="max-w-[1600px] mx-auto space-y-8 pb-10 px-4 sm:px-6">
      ...
      <header className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div className="space-y-1">
          ...
          <p className="text-muted-foreground">Real-time insights for Daar-ul-Maysaroh Management.</p>
        </div>

        <div className="flex items-center gap-3">
          <Button variant="outline" className="shadow-xs bg-background/50 backdrop-blur-xs">
          ...
          >
            <UserCheck className="mr-2 h-4 w-4" /> Review Requests
          </Button>
        </div>
      </header>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-8">
          <MainChartsSection charts={data.charts} />

          {/* APPROVALS SECTION */}
          <motion.div id="approvals" layout>
            <Card className="border-muted/50 shadow-xl shadow-slate-200/50 dark:shadow-none overflow-hidden">
              <CardHeader className="flex flex-row items-center justify-between border-b border-muted/20 bg-muted/5">
                <div>
                  <CardTitle>Registration Queue</CardTitle>
                  <CardDescription>New user requests awaiting verification</CardDescription>
                </div>
                <Badge variant={data.pendingUsers.length > 0 ? "default" : "secondary"}>
        ...
        <aside className="space-y-8">
          ...
          {/* Quick Stats / AI Insights Placeholders */}
          <div className="p-6 rounded-2xl bg-linear-to-br from-indigo-600 to-purple-700 text-white shadow-xl relative overflow-hidden group">
            <div className="absolute -right-6 -top-6 w-32 h-32 bg-white/10 rounded-full blur-3xl group-hover:scale-125 transition-transform" />
            <h4 className="font-bold text-lg mb-2 relative z-10">Advanced Analytics</h4>
            <p className="text-indigo-100 text-sm mb-4 relative z-10">Generate detailed attendance reports for the Ministry of Education.</p>
            <Button variant="secondary" size="sm" className="w-full font-semibold">Generate Report</Button>
          </div>
        </aside>
      </div>
    </div>
...
function EmptyQueueState() {
  return (
    <div className="flex flex-col items-center justify-center py-16 text-muted-foreground">
      ...
      <p className="text-lg font-medium text-slate-900 dark:text-white">Clean Slate!</p>
