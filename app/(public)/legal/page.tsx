...
const getColorStyles = (color: string) => {
  return {
    ...
    hover:
      color === "purple" ? "hover:border-purple-300" : "hover:border-amber-300",
    ring: color === "purple" ? "ring-purple-500/20" : "ring-amber-500/20",
  };
};
...
export default function LegalHub() {
  ...
  const handleCopyLink = () => {
    const url = `${window.location.origin}/legal#${activeTab}`;
    ...
    <div className="min-h-screen bg-gradient-to-b from-background via-purple-50/5 to-amber-50/5">
      ...
      {showPrintDialog && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center">
          <div className="bg-card rounded-xl p-6 max-w-sm mx-4 text-center">
            <Printer className="w-8 h-8 text-purple-600 mx-auto mb-3" />
            <h3 className="font-black mb-2">Preparing Document</h3>
            <p className="text-xs text-muted-foreground">
              Your document is being prepared for printing...
            </p>
          </div>
        </div>
      )}
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[10px] xs:text-xs text-muted-foreground flex items-center gap-2">
                  Version {version} · Last updated {lastUpdated}
                  <span className="hidden xs:inline">·</span>
                  <span className="inline-flex items-center gap-1">
                    <ShieldCheck className="w-2.5 h-2.5" />
                    Enforceable from {lastUpdated}
                  </span>
                </p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <button
                ...
                className="p-2 hover:bg-purple-50 dark:hover:bg-purple-950/20 rounded-lg transition-colors relative group"
        ...
        <div className="flex flex-col lg:flex-row gap-6 xs:gap-8">
            ...
            <div className="sticky top-28 space-y-4">
              ...
              <div className="relative">
                ...
                {searchTerm && (
                  ...
                  >
                    <X className="h-3 w-3 text-muted-foreground hover:text-purple-600" />
                  </button>
                )}
              </div>
          ...
          <main className="flex-1 min-w-0" ref={contentRef}>
            <div className="max-w-3xl mx-auto">
                ...
                <div className="flex items-center gap-3 xs:gap-4">
                  ...
                  <div>
                    <div className="flex flex-wrap items-center gap-2 mb-1">
                      ...
                      <span className="text-[9px] xs:text-[10px] text-purple-600 font-black">
                        Legally Enforceable
                      </span>
                    </div>
                    <h1 className="text-2xl xs:text-3xl font-black tracking-tighter">
              ...
              {/* Document Content */}
              <div
                className={`bg-card border ${activeColors.border} rounded-xl p-5 xs:p-6 sm:p-8 shadow-md`}
              >
                <div className="prose prose-sm dark:prose-invert max-w-none">
                  {formattedContent.map((block, idx) => {
                    if (block.type === "empty") {
                    ...
                    if (block.type === "point") {
                      return (
                        <div
                          key={idx}
                          className="flex gap-2 xs:gap-3 ml-3 xs:ml-4 my-1.5 xs:my-2 group"
                        >
                          <span
                            className={`text-[10px] xs:text-xs font-mono font-black ${activeColors.text} shrink-0`}
                          >
              ...
              <div className="mt-6 xs:mt-8">
                ...
                >
                  ...
                  <div className="mb-3 xs:mb-4 p-3 xs:p-4 bg-background rounded-lg border border-purple-200 dark:border-purple-800">
                    <div className="flex flex-col xs:flex-row items-start xs:items-center justify-between gap-2">
                      <div className="flex items-center gap-2 xs:gap-3">
                        <Fingerprint className="h-3.5 w-3.5 xs:h-4 xs:w-4 text-purple-600" />
                        <span className="text-[9px] xs:text-[10px] font-mono">
                          Document ID: ALM-LEGAL-{version.replace(/\./g, "")}
                        </span>
                      </div>
                      <div className="flex items-center gap-2">
              ...
              <div className="hidden print:block text-center text-[8px] text-muted-foreground mt-8 pt-4 border-t">
                <p>
                  Al-Maysaroh International Institute - Legal Framework v
                  {version}
                </p>
                <p>
                  Document ID: ALM-LEGAL-{version.replace(/\./g, "")} | Printed
