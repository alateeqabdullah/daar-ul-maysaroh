...
import {
  ...
  BookOpen, Download, LayoutGrid, List as ListIcon,
  Loader2, X, ChevronRight, Save
} from "lucide-react";
...
interface Props {
  ...
  classes: any[];
  subjects: any[]; // Make sure to pass subjects for filtering
  stats: any;
  pagination: any;
}

export default function TeacherAssignmentsClient({
  initialAssignments, classes, subjects, stats: serverStats
}: Props) {
  ...
  const [isEditing, setIsEditing] = useState(false); // Used to toggle Create vs Edit mode in Modal

  // Form Data
  const [formData, setFormData] = useState({
    title: "", description: "", classId: "", subjectId: "", 
    type: "HOMEWORK", totalMarks: "100", dueDate: ""
  });
  ...
  // Derived Subjects for Form (Filter subjects by selected Class)
  ...
  const handleCreateOrUpdate = async () => {
    ...
    try {
      ...
      if (isEditing) {
        setAssignments(prev => prev.map(a => a.id === selectedAssignment.id ? result.assignment : a));
  ...
  // --- HELPERS ---
  const resetForm = () => {
    setFormData({ title: "", description: "", classId: "", subjectId: "", type: "HOMEWORK", totalMarks: "100", dueDate: "" });
    setIsEditing(false);
  ...
  const openEditModal = (a: any) => {
    ...
    // Find classId from className if needed, or assume 'a' has it.
    // For this example, assuming 'a' is formatted from server with IDs or we can find it.
    // Ideally your server serializer should include raw IDs: classId, subjectId.
    // If not, we find it:
  ...
  const statCards = [
    { label: "Total Assignments", value: serverStats.total, icon: FileText, color: "from-blue-500 to-indigo-500", shadow: "shadow-blue-500/20" },
    { label: "Active", value: serverStats.active, icon: Calendar, color: "from-emerald-500 to-teal-500", shadow: "shadow-emerald-500/20" },
    { label: "Overdue", value: serverStats.overdue, icon: AlertCircle, color: "from-rose-500 to-red-500", shadow: "shadow-rose-500/20" },
    { label: "Submissions", value: serverStats.submissions, icon: CheckCircle2, color: "from-amber-400 to-orange-500", shadow: "shadow-amber-500/20" },
  ];
    ...
    <motion.div variants={containerVariants} initial="hidden" animate="show" className="space-y-8 pb-10">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Manage tasks, homework, and assessments</p>
        </div>
        <div className="flex gap-2">
           <div className="flex items-center bg-muted p-1 rounded-lg border">
              <Button size="icon" variant={viewMode === "grid" ? "white" : "ghost"} className={`h-7 w-7 rounded-md ${viewMode === 'grid' ? 'shadow-sm' : ''}`} onClick={() => setViewMode("grid")}><LayoutGrid className="h-4 w-4"/></Button>
              <Button size="icon" variant={viewMode === "list" ? "white" : "ghost"} className={`h-7 w-7 rounded-md ${viewMode === 'list' ? 'shadow-sm' : ''}`} onClick={() => setViewMode("list")}><ListIcon className="h-4 w-4"/></Button>
            </div>
          <Button variant="outline"><Download className="h-4 w-4 mr-2"/> Export</Button>
          <Button className="bg-gradient-to-r from-purple-600 to-indigo-600 text-white shadow-md hover:scale-105 transition-all gap-2" onClick={openAddModal}>
      ...
      <motion.div variants={itemVariants} className="flex flex-col sm:flex-row gap-4 bg-white/60 dark:bg-slate-900/60 backdrop-blur-md p-4 rounded-xl border shadow-sm">
        <div className="relative flex-1">
          ...
          <Input placeholder="Search assignments..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-9 bg-white dark:bg-slate-950 border-muted" />
        </div>
                    ...
                    <div className="flex justify-between items-start mb-4">
                       ...
                       <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                             <DropdownMenuItem onClick={(e) => { e.stopPropagation(); openEditModal(a); }}><Edit className="mr-2 h-4 w-4"/> Edit</DropdownMenuItem>
                             <DropdownMenuItem onClick={(e) => { e.stopPropagation(); handleDelete(a.id); }} className="text-red-600"><Trash2 className="mr-2 h-4 w-4"/> Delete</DropdownMenuItem>
                          </DropdownMenuContent>
                       </DropdownMenu>
                    </div>
      ...
      <AnimatePresence>
        {isAddModalOpen && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
             <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="bg-white dark:bg-slate-950 w-full max-w-lg rounded-2xl shadow-2xl border p-6 max-h-[90vh] overflow-y-auto">
                <div className="flex justify-between items-center mb-6">
                  <h2 className="text-xl font-bold">{isEditing ? "Edit Assignment" : "New Assignment"}</h2>
                  <Button size="icon" variant="ghost" onClick={() => setIsAddModalOpen(false)}><X className="h-5 w-5"/></Button>
                </div>
                <div className="grid gap-4">
                   <div className="space-y-1"><Label>Title</Label><Input value={formData.title} onChange={e => setFormData({...formData, title: e.target.value})} placeholder="e.g. Surah Al-Kahf Review" /></div>
                   <div className="space-y-1"><Label>Description</Label><Textarea value={formData.description} onChange={e => setFormData({...formData, description: e.target.value})} placeholder="Instructions..." className="min-h-[100px]" /></div>
                   ...
                   <div className="grid grid-cols-2 gap-4">
                      ...
                      <div className="space-y-1"><Label>Subject</Label>
                        <Select value={formData.subjectId} onValueChange={v => setFormData({...formData, subjectId: v})} disabled={!formData.classId}>
                          ...
                          <SelectContent>
                            {availableSubjects.length > 0 ? availableSubjects.map((s:any) => <SelectItem key={s.id} value={s.id}>{s.name}</SelectItem>) 
                   ...
                   <Button className="w-full mt-4 bg-purple-600 hover:bg-purple-700 text-white" onClick={handleCreateOrUpdate} disabled={isLoading}>
                     {isLoading ? <Loader2 className="animate-spin mr-2"/> : isEditing ? "Save Changes" : "Create Task"}
                   </Button>
                </div>
             </motion.div>
          </motion.div>
