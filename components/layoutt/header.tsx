    ...
    <>
      ...
      >
        <nav className="container mx-auto px-4 md:px-6 flex items-center justify-between">
          ...
          {/* --- DESKTOP NAVIGATION - ENHANCED COLORS --- */}
          <ul className="hidden xl:flex items-center space-x-1">
            {navigation.map((item) => {
              const isActive = pathname === item.href;
              ...
              return (
                ...
                >
                  ...
                  {/* MEGA MENU DROPDOWN - ENHANCED WITH PURPLE/GOLD */}
                  <AnimatePresence>
                    {item.dropdown && activeDropdown === item.name && (
                      <motion.div
                        initial={{ opacity: 0, y: 10, scale: 0.95 }}
          ...
          <div className="flex items-center space-x-2 md:space-x-3 lg:space-x-4 relative z-60">
            {/* Free Assessment Button - NEW */}
            {/* <Link href="/assessment" className="hidden md:block">
              <Button className="h-10 px-4 rounded-full bg-transparent border border-amber-500 text-amber-500 hover:bg-amber-500 hover:text-white text-[10px] font-bold transition-all duration-300">
                <Calendar className="w-3.5 h-3.5 mr-1.5" />
                Free Assessment
              </Button>
            </Link> */}

            {/* Theme Toggle - Enhanced colors */}
            <Button
              variant="ghost"
              size="icon"
              onClick={() => setTheme(theme === "dark" ? "light" : "dark")}
              ...
              {session ? (
                ...
                <>
                  <Link href="/login">
                    <Button
                      variant="ghost"
                      className="font-black text-[11px] tracking-widest uppercase px-4 min-h-11 text-slate-300 hover:text-white"
                    >
                      LOGIN
                    </Button>
                  </Link>
      ...
      {/* --- MOBILE DRAWER - KEPT YOUR STRUCTURE, ENHANCED COLORS --- */}
      <AnimatePresence>
        {mobileMenuOpen && (
          <>
            <motion.div
              ...
              <div className="mt-auto pt-6 space-y-4 pb-safe">
                ...
                {session ? (
                  <div className="grid grid-cols-1 gap-3">
                    ...
                    <Button
                      variant="outline"
                      className="w-full h-14 rounded-2xl font-black border-2 border-red-500/30 text-red-400 hover:bg-red-500/10"
                      onClick={() => {
                        signOut();
                        setMobileMenuOpen(false);
                      }}
                  ...
                  <div className="grid grid-cols-1 gap-4">
                    ...
                    >
                      <Button
                        variant="outline"
                        className="w-full h-14 rounded-2xl font-black border-2 border-purple-600/30 text-slate-300"
                      >
                        SIGN IN
                      </Button>
                    </Link>
