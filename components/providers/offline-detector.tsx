...
import { useEffect, useState, useCallback, useRef } from "react";
import { motion, AnimatePresence } from "framer-motion";
import { WifiOff, X, RefreshCw, Wifi, Battery, Zap } from "lucide-react";
import { Button } from "@/components/ui/button";
import { cn } from "@/lib/utils";
import { toast } from "sonner";
...
export function OfflineProvider({
  ...
  return (
    <>
      ...
      <AnimatePresence>
        {isOffline && showBanner && (
          <motion.div
            ...
            className="fixed inset-x-0 bottom-0 sm:bottom-4 sm:inset-x-auto sm:left-1/2 sm:-translate-x-1/2 z-50 px-3 sm:px-0"
          >
            <div className="bg-gradient-to-r from-amber-500/95 to-orange-500/95 dark:from-amber-950/95 dark:to-orange-950/95 backdrop-blur-md border border-amber-500/30 rounded-2xl shadow-2xl overflow-hidden w-full sm:w-[440px] max-w-full">
              ...
              <div className="p-4 sm:p-5">
                <div className="flex items-start gap-3 sm:gap-4">
                  ...
                  <button
                    onClick={handleDismiss}
                    className="shrink-0 p-1.5 sm:p-2 -mt-1 -mr-1 rounded-full hover:bg-white/10 transition-colors"
                  >
                    <X className="w-4 h-4 sm:w-5 sm:h-5 text-muted-foreground" />
                  </button>
                </div>
      ...
      <AnimatePresence>
        {!isOffline && isReconnecting && (
          <motion.div
            ...
            className="fixed top-4 sm:top-6 left-1/2 -translate-x-1/2 z-50"
          >
            <div className="bg-gradient-to-r from-green-500/95 to-emerald-500/95 dark:from-green-950/95 dark:to-emerald-950/95 backdrop-blur-md border border-green-500/30 rounded-full px-5 py-2.5 shadow-lg">
      ...
      {/* BACK ONLINE TOAST */}
      <AnimatePresence>
        {!isOffline && !isReconnecting && hasBeenOffline && !showBanner && (
          <motion.div
            initial={{ opacity: 0, y: -20 }}
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: -20 }}
            ...
            className="fixed top-4 sm:top-6 left-1/2 -translate-x-1/2 z-50"
          >
            <div className="bg-gradient-to-r from-green-500 to-emerald-500 rounded-full px-5 py-2.5 shadow-lg">
              <div className="flex items-center gap-2">
