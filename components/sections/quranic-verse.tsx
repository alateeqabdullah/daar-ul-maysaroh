...
// components/shared/QuranicVerse.tsx
"use client";
...
// Extended verse data - can be expanded
const VERSE_LIBRARY: Record<string, any> = {
  "15:9": {
    surah: 15,
    ayah: 9,
    surahName: "Al-Hijr",
  ...
  "20:114": {
    ...
    reflection: "A timeless dua for seeking knowledge and wisdom.",
  ...
  "54:17": {
    ...
    transliteration: "Wa laqad yassarna al-qur'ana lidhikri fahal min muddakir",
    reflection: "The Quran is made easy to remember and reflect upon.",
    theme: "Ease of Remembrance",
  },
};
...
export function QuranicVerse({
  ...
  // Get verse data - from library or fallback
  const verseData = useMemo(() => {
    if (verseKey && VERSE_LIBRARY[verseKey]) {
      return {
        ...VERSE_LIBRARY[verseKey],
        url: `https://quran.com/${VERSE_LIBRARY[verseKey].surah}/${VERSE_LIBRARY[verseKey].ayah}`,
      };
    }
  ...
  const shareVerse = async () => {
    const shareText = `"${verseData.translation}" — Surah ${verseData.surahName} (${verseData.ayah})\n\n${verseData.url}`;
    try {
      if (navigator.share) {
        await navigator.share({
          title: `Surah ${verseData.surahName} - Verse ${verseData.ayah}`,
          text: shareText,
          url: verseData.url,
        });
      } else {
        await navigator.clipboard.writeText(shareText);
        toast.success("Verse link copied to clipboard!");
      }
    } catch (err) {
  ...
  const likeVerse = () => {
    setIsLiked(!isLiked);
    toast.success(
      isLiked ? "Removed from favorites" : "Added to favorites! ❤️",
    );
  };
      ...
      <div className="container mx-auto px-4 xs:px-5 sm:px-6 relative z-10 text-center space-y-6 xs:space-y-8 sm:space-y-10 md:space-y-12">
        <motion.div style={{ scale, opacity, y }}>
          ...
          <div className="inline-flex items-center gap-1.5 xs:gap-2 px-2.5 xs:px-3 py-1 xs:py-1.5 rounded-full bg-purple-100 dark:bg-purple-950/30 border border-purple-200 dark:border-purple-800 mb-4 xs:mb-6 sm:mb-8">
            ...
            <div className="w-px h-3 bg-purple-300/50" />
            <span className="text-[8px] xs:text-[9px] font-black text-amber-500">
              {verseData.theme || "Divine Word"}
            </span>
          </div>

          {/* Arabic Verse - Premium Styling */}
          <div
            className="quran-monumental mb-4 xs:mb-6 sm:mb-8 md:mb-10 drop-shadow-[0_0_30px_rgba(139,92,246,0.15)] relative"
            dir="rtl"
            lang="ar"
          ...
          >
            <div className="relative inline-block">
              ...
              <div className="absolute -inset-4 border border-purple-600/10 rounded-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-500" />

              <span className="text-2xl xs:text-3xl sm:text-5xl md:text-7xl lg:text-8xl xl:text-9xl tracking-wider leading-relaxed block wrap-break-word px-2 text-purple-900 dark:text-purple-100 font-bold">
                {verseData.arabic.split(" ").slice(0, 5).join(" ")}
              </span>
              <span className="text-2xl xs:text-3xl sm:text-5xl md:text-7xl lg:text-8xl xl:text-9xl tracking-wider leading-relaxed block mt-1 xs:mt-2 wrap-break-word px-2 text-purple-900 dark:text-purple-100 font-bold">
                {verseData.arabic.split(" ").slice(5).join(" ")}
              </span>
            </div>

            <span className="text-xl xs:text-2xl sm:text-3xl md:text-4xl lg:text-5xl text-amber-500 dark:text-amber-400 block mt-2 xs:mt-3 sm:mt-4 font-bold">
              ۝ {ayah}
            </span>

            {/* Decorative dots - Premium Animation */}
            <div className="absolute -bottom-4 xs:-bottom-5 sm:-bottom-6 left-1/2 transform -translate-x-1/2 flex gap-1.5 xs:gap-2">
              {[1, 2, 3].map((dot) => (
                <motion.div
                  key={dot}
                  className="w-1 h-1 xs:w-1.5 xs:h-1.5 rounded-full bg-purple-500/40"
          ...
          <div className="max-w-4xl mx-auto space-y-4 xs:space-y-5 sm:space-y-6 md:space-y-8 px-4">
            ...
            >
              ...
              <div className="flex flex-col items-center gap-0.5 xs:gap-1 sm:gap-2">
                <p className="text-[8px] xs:text-[9px] sm:text-[10px] md:text-xs font-black text-amber-600 dark:text-amber-400 uppercase tracking-[0.2em] sm:tracking-[0.3em] md:tracking-[0.4em]">
                  {verseData.theme || "Divine Promise"}
                </p>
              </div>
              <div
                className={`${dividerGradient} w-8 xs:w-10 sm:w-12 md:w-20`}
            ...
            <div className="flex flex-wrap items-center justify-center gap-2 xs:gap-3 sm:gap-4 mt-4 xs:mt-5 sm:mt-6 md:mt-8">
              {showTafsir && (
                <motion.button
                  whileHover={{ scale: 1.02, y: -2 }}
                  whileTap={{ scale: 0.98 }}
                  className="px-5 xs:px-6 sm:px-7 md:px-8 py-2 xs:py-2.5 sm:py-3 rounded-full bg-gradient-to-r from-purple-600 to-purple-700 hover:from-purple-700 hover:to-purple-800 text-white font-black text-[10px] xs:text-xs sm:text-sm transition-all duration-300 shadow-md hover:shadow-lg w-full xs:w-auto group"
                  aria-label="Read Tafsir explanation"
                  onClick={() => window.open(verseData.url, "_blank")}
                >
                  <span className="flex items-center justify-center gap-1.5 xs:gap-2">
              ...
              {showCopy && (
                <motion.button
                  whileHover={{ scale: 1.02, y: -2 }}
                  whileTap={{ scale: 0.98 }}
                  className="px-5 xs:px-6 sm:px-7 md:px-8 py-2 xs:py-2.5 sm:py-3 rounded-full border-2 border-purple-300 dark:border-purple-700 text-purple-700 dark:text-purple-400 font-black text-[10px] xs:text-xs sm:text-sm hover:bg-purple-50 dark:hover:bg-purple-950/30 transition-all duration-300 w-full xs:w-auto group"
                  aria-label="Copy verse to clipboard"
                  onClick={copyVerse}
                >
                  <span className="flex items-center justify-center gap-1.5 xs:gap-2">
              ...
              {showShare && (
                <motion.button
                  whileHover={{ scale: 1.02, y: -2 }}
                  whileTap={{ scale: 0.98 }}
                  className="px-4 xs:px-5 py-2 xs:py-2.5 rounded-full border-2 border-amber-300 dark:border-amber-700 text-amber-600 dark:text-amber-400 font-black text-[10px] xs:text-xs sm:text-sm hover:bg-amber-50 dark:hover:bg-amber-950/30 transition-all duration-300"
              ...
              {showContact && (
                <motion.button
                  whileHover={{ scale: 1.02, y: -2 }}
                  whileTap={{ scale: 0.98 }}
                  className="px-4 xs:px-5 py-2 xs:py-2.5 rounded-full border-2 border-emerald-300 dark:border-emerald-700 text-emerald-600 dark:text-emerald-400 font-black text-[10px] xs:text-xs sm:text-sm hover:bg-emerald-50 dark:hover:bg-emerald-950/30 transition-all duration-300"
                  aria-label="Contact us about this verse"
            ...
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              className="inline-flex items-center gap-2 text-xs text-muted-foreground hover:text-purple-600 transition-colors mt-2"
              onClick={likeVerse}
            >
              <Heart
                className={`w-4 h-4 ${isLiked ? "fill-red-500 text-red-500" : ""}`}
              />
              {isLiked ? "Saved to favorites" : "Save this verse"}
            </motion.button>
          </div>
        </motion.div>
        ...
        <div className="relative z-10 mt-8">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-purple-50/30 dark:bg-purple-950/20 border border-purple-200 dark:border-purple-800">
            <Sparkles className="w-3.5 h-3.5 text-amber-500" />
            <span className="text-[8px] xs:text-[9px] font-black uppercase tracking-wider text-muted-foreground">
              Share the light of the Quran
            </span>
          </div>
        </div>
      </div>
