...
const TRUST_SEALS = [
  {
    ...
    sub: "Verified Sanad Chains",
    description:
      "Direct transmission chains from certified scholars with documented isnad reaching back 1,400 years to Prophet Muhammad (ﷺ)",
    color: "from-primary-600 to-primary-800",
    bgColor: "primary",
    verifyUrl: "/certifications/ijazah",
  },
  {
    id: "al-azhar",
    icon: Landmark,
    ...
    sub: "Al-Azhar Affiliated",
    description:
      "Faculty certified by Al-Azhar University with international recognition and ijazah in classical Islamic sciences",
    color: "from-primary-500 to-primary-700",
    bgColor: "primary",
    verifyUrl: "/certifications/al-azhar",
  },
  {
    id: "pedagogy",
    icon: Award,
    ...
    sub: "1-on-1 Academic Focus",
    description:
      "Personalized learning with dedicated master teachers, progress tracking, and customized lesson plans for each student",
    color: "from-gold/90 to-gold",
    bgColor: "gold",
    verifyUrl: "/methodology",
  },
  {
    id: "traditional",
    icon: ScrollText,
    ...
    sub: "Classical Methodology",
    description:
      "Study of classical Islamic sciences following authentic scholarly traditions, including Tuhfatul Atfal and Jazariyyah",
    color: "from-accent to-accent/90",
    bgColor: "accent",
    verifyUrl: "/curriculum/traditional",
  },
];
...
export function TrustIndicators() {
  ...
  const itemVariants = {
    ...
    visible: {
      ...
      transition: {
        ...
        stiffness: reduceMotion ? 0 : 100,
        damping: reduceMotion ? 0 : 15,
    ...
    hover: {
      ...
      transition: {
        type: reduceMotion ? "tween" : "spring",
        stiffness: reduceMotion ? 0 : 400,
        damping: reduceMotion ? 0 : 25,
        duration: 0.3,
      },
    },
  };
  ...
  const iconVariants = {
    ...
    visible: {
      ...
      transition: {
        type: reduceMotion ? "tween" : "spring",
        stiffness: reduceMotion ? 0 : 200,
        damping: reduceMotion ? 0 : 15,
        duration: reduceMotion ? 0.3 : 0.6,
      },
    },
    hover: {
      rotate: reduceMotion ? 0 : [0, -10, 10, 0],
      transition: {
    ...
    >
      ...
      <div className="container mx-auto px-4 sm:px-6 relative z-10">
        ...
        >
          ...
          <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-black tracking-tighter leading-[0.9] md:leading-[0.8] font-heading mb-4">
            Trusted by{" "}
            <span className="bg-gradient-to-r from-primary-600 via-primary-700 to-primary-800 bg-clip-text text-transparent italic">
              Scholars
            </span>{" "}
            Worldwide
          </h2>
        ...
        >
          {TRUST_SEALS.map((seal, i) => (
            <motion.div
              ...
              tabIndex={0}
              role="article"
              aria-label={`${seal.label}: ${seal.sub}`}
            >
              {/* Glow Effect */}
              {!reduceMotion && (
                <motion.div
              ...
              {/* Main Card */}
              <div className="relative bg-gradient-to-br from-card to-card/80 backdrop-blur-sm border border-border/50 dark:border-white/5 rounded-2xl p-5 sm:p-6 md:p-7 lg:p-8 shadow-lg shadow-black/5 dark:shadow-black/20 overflow-hidden h-full focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2">
                {/* Corner Accent */}
                <div
                  className={`absolute top-0 right-0 w-12 h-12 sm:w-14 sm:h-14 md:w-16 md:h-16 bg-gradient-to-bl ${seal.color} opacity-5 rounded-bl-full`}
                />
                ...
                <div className="relative mb-5 sm:mb-6">
                  <motion.div
                    variants={iconVariants}
                    className={`relative w-12 h-12 sm:w-14 sm:h-14 md:w-16 md:h-16 rounded-xl bg-gradient-to-br ${seal.color} flex items-center justify-center shadow-lg`}
                    aria-hidden="true"
                  >
                    <seal.icon className="w-6 h-6 sm:w-7 sm:h-7 md:w-8 md:h-8 text-white" />
                  ...
                  {!reduceMotion && (
                    <motion.div
                      ...
                      className="absolute -top-2 -right-2 w-5 h-5 sm:w-6 sm:h-6 rounded-full bg-background border border-white/20 flex items-center justify-center shadow-md"
                      aria-label="Verified"
                    >
                      <CheckCircle className="w-3 h-3 text-emerald-500" />
                    </motion.div>
                ...
                <div className="space-y-3 sm:space-y-4">
                  ...
                  <p
                    className={`text-sm sm:text-base font-bold uppercase tracking-widest bg-gradient-to-r ${seal.color} bg-clip-text text-transparent`}
                  >
                    {seal.sub}
                  </p>
                  <p className="text-sm text-muted-foreground opacity-90 leading-relaxed">
                  ...
                  {/* Verify Link */}
                  <button
                    onClick={() => window.open(seal.verifyUrl, "_blank")}
                    className="flex items-center gap-1 text-xs sm:text-sm text-primary-700 hover:text-primary-800 dark:text-primary-400 dark:hover:text-primary-300 font-medium underline underline-offset-2 transition-colors mt-2 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 focus:rounded"
                    aria-label={`Verify ${seal.label} certification`}
                  >
              ...
              {!reduceMotion && (
                <motion.div
                  initial={{ opacity: 0, y: 10, scale: 0.9 }}
                  whileHover={{ opacity: 1, y: 0, scale: 1 }}
                  className="absolute -top-3 left-1/2 -translate-x-1/2 z-20 px-2 py-1 sm:px-3 sm:py-1.5 rounded-full bg-background border border-white/10 shadow-lg"
                  role="status"
                  aria-live="polite"
                >
                  <span className="text-[10px] sm:text-xs font-black uppercase tracking-[0.2em] text-primary-700 dark:text-primary-400 whitespace-nowrap">
                    Certified
                  </span>
                </motion.div>
              )}
        ...
        >
          <div className="inline-flex items-center gap-3 text-sm text-muted-foreground">
            <div className="h-px w-6 sm:w-8 bg-gradient-to-r from-transparent to-border" />
            <span className="font-medium">
              All certifications verified annually by independent scholars
            </span>
            <div className="h-px w-6 sm:w-8 bg-gradient-to-l from-transparent to-border" />
          </div>
          ...
          <div className="mt-6">
            <a
              href="/certifications"
              className="inline-flex items-center gap-2 px-6 py-3 rounded-full bg-primary-700 hover:bg-primary-800 text-white font-black text-sm uppercase tracking-widest transition-colors focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 min-h-[44px]"
              aria-label="View all certifications and verifications"
            >
