...
export function Hero({ className }: HeroProps) {
  ...
  return (
    <section
      ref={containerRef}
      className={cn(
        "relative min-h-screen md:min-h-[110vh] flex items-center pt-20 md:pt-32 pb-10 md:pb-20 overflow-hidden bg-background",
        className,
      )}
    >
      {/* --- BACKGROUND --- */}
      <motion.div style={{ y: bgYValue }} className="absolute inset-0 z-0">
        ...
        <motion.div
          ...
          className="absolute top-1/4 left-1/2 md:left-1/3 w-[300px] md:w-[800px] h-[300px] md:h-[800px] bg-purple-600/5 md:bg-purple-600/10 blur-[60px] md:blur-[160px] rounded-full pointer-events-none -translate-x-1/2 md:translate-x-0"
        />
        ...
        {[...Array(isMobile ? 3 : 6)].map((_, i) => (
          <motion.div
            ...
            className="absolute w-1.5 h-1.5 md:w-2 md:h-2 bg-amber-500/30 md:bg-amber-500/40 rounded-full blur-[1px] md:blur-[2px]"
            style={{
              left: `${isMobile ? 10 + i * 25 : 15 + i * 15}%`,
              top: `${isMobile ? 30 + i * 15 : 20 + i * 10}%`,
            }}
      ...
      <div className="container mx-auto px-4 xs:px-5 sm:px-6 lg:grid lg:grid-cols-2 gap-8 md:gap-16 xl:gap-24 items-center relative z-10">
        {/* --- LEFT: CONTENT --- */}
        <motion.div
          style={{ y: textYValue }}
          initial={{ opacity: 0, y: 20 }}
        ...
        >
          ...
          <div className="relative pt-16 inline-block">
            <motion.div
              whileHover={!isMobile ? { scale: 1.05 } : {}}
              className="relative z-10 inline-flex items-center gap-2 md:gap-3 px-4 md:px-6 py-2 md:py-2.5 rounded-xl md:rounded-2xl glass-surface border-white/10 md:border-white/20 text-purple-700 text-[10px] md:text-[11px] font-bold md:font-black tracking-[0.2em] md:tracking-[0.3em] uppercase shadow-lg md:shadow-2xl"
            >
              <div className="w-1.5 h-1.5 md:w-2 md:h-2 rounded-full bg-emerald-500 animate-pulse" />
              <span className="whitespace-nowrap">AL-MAYSAROH INSTITUTE</span>
            </motion.div>
            <div className="absolute inset-0 bg-purple-600/10 md:bg-purple-600/20 blur-lg md:blur-xl rounded-full" />
          </div>

          {/* HEADLINE */}
          <div className="space-y-4 md:space-y-5">
            <h1 className="text-4xl xs:text-5xl sm:text-6xl md:text-7xl lg:text-8xl font-black tracking-tighter leading-[1.1] font-heading">
              Not Just Quran Classes. <br />
            ...
            <p className="text-sm md:text-base text-muted-foreground/80 border-l-2 border-amber-500/50 pl-4 py-1">
              <span className="font-semibold">1400-year unbroken chain</span> to
              Prophet Muhammad (ﷺ) • 1-on-1 with certified scholars
            </p>
          </div>
            ...
            <div className="flex items-center gap-3">
              <div className="flex -space-x-3">
                {[...Array(4)].map((_, i) => (
                  <div
                    key={i}
                    className="w-8 h-8 md:w-10 md:h-10 rounded-full bg-gradient-to-br from-purple-600 to-purple-800 border-2 border-background shadow-lg"
                  />
                ))}
              </div>
              <div className="text-sm md:text-base">
          ...
          {/* CALL TO ACTION */}
          <div className="flex flex-col sm:flex-row flex-wrap gap-4 md:gap-6 pt-4 pb-8">
            <Link href="/courses" className="w-full sm:w-auto">
              <Button
                size={isMobile ? "default" : "lg"}
                className="h-14 sm:h-16 md:h-20 w-full sm:w-auto px-6 sm:px-8 md:px-12 rounded-3xl md:rounded-4xl text-sm md:text-md font-bold md:font-black bg-purple-700 hover:bg-purple-800 text-white shadow-lg md:shadow-[0_30px_60px_-15px_rgba(124,58,237,0.4)] transition-all group overflow-hidden relative min-h-[44px]"
              >
                <span className="relative z-10 flex items-center justify-center sm:justify-start gap-2">
                  SCHEDULE ASSESSMENT{" "}
                  <ArrowRight className="w-4 h-4 md:w-6 md:h-6 group-hover:translate-x-1 transition-all" />
            ...
            <div className="flex items-center gap-4 md:gap-6 px-0 sm:px-4">
              <div className="h-8 md:h-12 w-px bg-border/50" />
              <div className="flex flex-col">
                <p className="text-[10px] md:text-[10px] font-bold md:font-black tracking-wider md:tracking-widest text-amber-500 uppercase">
                  Free Assessment
                </p>
                <p className="font-semibold md:font-bold text-sm">
                  No obligation
      ...
      {!isMobile && shouldAnimate && (
        ...
        >
          ...
          <div className="w-px h-8 md:h-12 bg-gradient-to-b from-purple-700 to-transparent" />
        </motion.div>
      )}
    </section>
  );
